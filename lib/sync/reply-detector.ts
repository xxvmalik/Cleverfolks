...
export async function detectPipelineReply(
  ...
  try {
    console.log(`[reply-detector] v3 — entering detectPipelineReply`);
    ...
    // ── Guard 0: Fetch workspace owner email (used by multiple guards) ────
    ...
    console.log(`[reply-detector] Guard 0 — ownerEmail: ${ownerEmail}, senderEmail: ${senderEmail}`);

    // Self-send check: if sender IS the workspace owner, this is an outgoing
    // email (e.g. from Sent Items), not a reply from the lead.
    if (ownerEmail && senderEmail === ownerEmail) {
      console.log(`[reply-detector] Skipping — sender ${senderEmail} IS the workspace owner (outgoing email)`);
      return { is_reply: false };
    }
