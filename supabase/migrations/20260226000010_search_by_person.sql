-- ============================================================
-- Sprint 4.5: search_by_person
-- Full-text + metadata search scoped to a specific person.
-- Matches chunk_text, metadata->user_name, metadata->user.
-- ============================================================
...
AS $$
  ...
  WHERE dc.workspace_id = p_workspace_id
    AND (
      dc.chunk_text          ILIKE '%' || p_person_name || '%'
      OR dc.metadata->>'user_name' ILIKE '%' || p_person_name || '%'
